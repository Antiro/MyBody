...
$ApServices = $dataAdmin->getAllApplicationService();
...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Заявки на услуги</h1>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="card">
                    <div class="card-body table-responsive p-0" style="max-height: 450px;">
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                            <?php foreach ($ApServices as $s): ?>
                                <tr data-widget="expandable-table" aria-expanded="false">
                                    <td class="text-center"><?= $s->id ?></td>
                                    <td class="text-center"><?= $dataAdmin->getOneGym($s->id_gym)->address ?></td>
                                    <td class="text-center"><?= $s->name ?></td>
                                    <td class="text-center"><?= $s->email ?></td>
                                    <td class="text-center"><?= $dataAdmin->getOneService($s->id_services)->title ?></td>
                                    <td class="text-center"><?= $s->phone ?></td>
                                    <td class="project-actions text-center">
                                        <a href="/layouts/panel/application/applicationService/delete.php?id=<?= $s->id ?>"
