...
<?php include_once $_SERVER['DOCUMENT_ROOT'] . "/layouts/nav.php"; ?>
    ...
    <div class="pricing-header px-3 py-3 pt-md-5 pb-md-4 mx-auto text-center">
        <h1 class="display-4">Заявка на услугу</h1>
    </div>
    <div class="container">
        <div class="card">
            <div class="card-body text-center">
                <form action="/application_service/add.php" method="get">
                    <div>
                        <label for="id_services" class="w-50">Услуга<br>
                            <select name="id_services" class="form-control">
                                <?php foreach ($allServices as $k): ?>
                                    <option value="<?= $k->id ?>"
                                            <?php if ($services->id == $k->id): ?>selected<?php endif; ?>>
                                        <?= $k->title ?>
                                    </option>
                                <?php endforeach ?>
                            </select>
